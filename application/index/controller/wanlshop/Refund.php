...
class Refund extends Wanlshop
{
	...
	public function agree($ids = null)
	{
		...
		try {
			...
			// 推送开始
			$this->pushRefund($row['id'], $row['order_id'], $row['goods_ids'], $push_title);
			// 更新退款
	...
	public function receiving($ids = null)
	{
		...
		try {
			...
			$this->setRefundState($row['order_id']);
			// 推送开始
			$this->pushRefund($row['id'], $row['order_id'], $row['goods_ids'], '退款已完成');
			// 更新退款
		...
		if ($this->request->isPost()) {
		    ...
		    if ($params) {
		        ...
		        try {
					...
					$this->setRefundState($row['order_id']);
					// 推送开始
					$this->pushRefund($row['id'], $row['order_id'], $row['goods_ids'], '退款申请被拒绝');
					// 更新退款
